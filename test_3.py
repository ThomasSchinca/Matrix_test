...
res_tot = pd.read_csv('res_matrix.csv',index_col=0)

def func(x, a,b,c):
    return a * np.exp(-b * x) + c
popt, pcov = curve_fit(func, res_tot.n_mot+1, res_tot.iloc[:,:5].mean(axis=1))

plt.scatter(res_tot.n_mot+1,res_tot.iloc[:,:5].mean(axis=1))
plt.plot(range(0,21), func(range(0,21), *popt),'g-',)
plt.ylim(-500,800)
plt.xlim(0,20)
plt.hlines(0,0,20,color='red')
plt.ylabel('MAE forecasting improv')
plt.xlabel('Number of other seq in motif')
plt.show()

good = res_tot[(res_tot.H1>0) & (res_tot.H2>0) & (res_tot.H3>0) & (res_tot.H4>0) & (res_tot.H5>0) & (res_tot.n_mot>=12)]
...
for row in range(len(win_sel)):
    ...
    h_max=5
    prof = mp.compute(n_test,windows=[win,200])
    profile = mp.discover.motifs(prof,k=20,max_neighbors=100,radius=rad_sel[row])
    pmp= profile['pmp'][0,:]
    to=0
    exclude=[]
    for i in seq_n:
    ...
    tab_mot = n_test[ind_m[0]:ind_m[0]+win].reshape((win,1))
    for i in ind_m[1:]:
        try:
            tab_mot=np.concatenate([tab_mot,n_test[i:i+win].reshape((win,1))],axis=1)
        except:
            1
    scaler = MinMaxScaler()
    ...
    plt.figure(figsize=(8,6))
    for i in range(len(tab_mot.columns)):
        plt.plot(tab_mot.iloc[:,i],color='blue',alpha=0.1)
    plt.plot(m_tab.iloc[:],color='blue',linewidth=2)    
    plt.fill_between([*range(win)],m_tab-std_tab,m_tab+std_tab,color='blue',alpha=0.2)      
    ...
    for i in ind_m:
        try:
            ...
            if ((len(ind_m)%3==0) & (len(ind_m)//3==1)) | (len(ind_m)//3==0) :
                ax[c_l].plot(obs,label='Observed value',marker='o')
                ax[c_l].plot(pred_mp_p,label='MP predict',marker='o')
                ax[c_l].plot(pred_ar_p,label='AR predict',marker='o')
                ax[c_l].vlines(obs.index[win-1],0,obs.max()+0.1*obs.max(),colors='red',linestyles='--')
                ax[c_l].legend()
                ax[c_l].set_title(col)
            else:
...
thres = res_tot[res_tot.n_mot>1]
thres.index = range(len(thres))   
 
plt.scatter(thres.n_mot+1,thres.iloc[:,:5].mean(axis=1))
plt.plot(range(2,21), func(range(2,21), *popt),'g-',)
plt.ylim(-500,800)
plt.xlim(0,20)
plt.hlines(0,0,20,color='red')
plt.ylabel('MAE forecasting improv')
...
for i in thres.rad.unique():
    df_t= thres[thres.rad==i]
    plt.plot(df_t.iloc[:,:5].mean(axis=0),label=str(i),marker='o',linewidth=0)
plt.hlines(0,-0.5,4.5,linestyles='--',colors='red')
plt.legend()
plt.show()
